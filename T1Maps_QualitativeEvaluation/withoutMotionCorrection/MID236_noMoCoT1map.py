...
This script generates the uncorrected T1 map for the volunteer MID236.
...
filepath_data = "/data/zhang18/MRI_Data/Data_PTBMR04_012/meas_MID236_T1resp_13x13x8_FID74934.h5"
...
mr.Pars.Nav.x_line = 50
mr.Pars.Nav.y_min = 50
mr.Pars.Nav.y_max = 100
mr.Pars.Nav.smooth_fac = 75
...
'''Do T1 fitting without motion correction'''
...
mr.ReconData(mr)
mr.CombineCoils(mr)
idat_ti_noMoCo = np.squeeze(mr.Data.I)

# T1 fit of continuous radial acq - multiple CPU
mr.Pars.Fit.FitPerformance = 'multi'
mr.FitImages(mr)
idat_qpar_noMoCo = np.squeeze(mr.Data.Fit)
rec_im.append(np.concatenate(r1_to_t1_rgb(mr.Data.Fit), axis=0))
fig1 = plt.figure()
im1 = plt.imshow((1/abs(idat_qpar_noMoCo[:,:,2]))*1000, cmap = 'magma', vmin = 0, vmax = 2500) # also check on other sequential cmaps
fig1.colorbar(im1)